...
#include "coll_all_gather_double_ring_asym_executor.h"

namespace hccl {

CollAllGatherDoubleRingAsymExecutor::CollAllGatherDoubleRingAsymExecutor(const HcclDispatcher dispatcher,
...
HcclResult CollAllGatherDoubleRingAsymExecutor::CalcStreamNum(u32& streamNum)
{
    ...
    HCCL_INFO("[CollAllGatherDoubleRingAsymExecutor][CalcStreamNum] tag[%s] streamNum_[%u]",
        tag_.c_str(), streamNum);
    return HCCL_SUCCESS;
}

HcclResult CollAllGatherDoubleRingAsymExecutor::CalcCommInfo(std::vector<LevelNSubCommTransport>& opTransport)
{
...
HcclResult CollAllGatherDoubleRingAsymExecutor::CalcTransportMemType(TransportMemType &inputType,
    TransportMemType &outputType)
{
    ...
    HCCL_INFO("[CollAllGatherDoubleRingAsymExecutor][CalcTransportMemType] tag[%s] inputType[%d], outputType[%d]",
        tag_.c_str(), inputType, outputType);
    return HCCL_SUCCESS;
}

HcclResult CollAllGatherDoubleRingAsymExecutor::CalcLevel0CommInfo(TransportMemType inputType,
    TransportMemType outputType,
...
HcclResult CollAllGatherDoubleRingAsymExecutor::CalcLevel2CommInfo(TransportMemType inputType,
    TransportMemType outputType,
    std::vector<LevelNSubCommTransport>& opTransport)
{
...
u64 CollAllGatherDoubleRingAsymExecutor::CalcLoopMaxCount(const u64 cclBuffSize, const u32 unitSize)
{
    u64 maxCountPerLoop = cclBuffSize / (topoAttr_.userRankSize * unitSize);
    return maxCountPerLoop;
}

bool CollAllGatherDoubleRingAsymExecutor::IsDataSplitForRdmaSdmaConcurrent(const u64 curSize)
{
...
HcclResult CollAllGatherDoubleRingAsymExecutor::KernelRun(const OpParam &param, ExecMem &execMem)
{
    ...
    CHK_PRT_RET(perDataSize == 0,
        HCCL_ERROR("[CollAllGatherDoubleRingAsymExecutor][KernelRun]errNo[0x%016llx] datatype[%d] is invalid",
    ...
    if (!DMAReduceFlag_) {
        ...
        CHK_PRT_RET(ret != HCCL_SUCCESS,
            HCCL_ERROR("[CollAllGatherDoubleRingAsymExecutor][KernelRun]all gather double "
    ...
    if (topoAttr_.superPodNum > 1) {
        ...
        CHK_PRT_RET(ret != HCCL_SUCCESS,
            HCCL_ERROR("[CollAllGatherDoubleRingAsymExecutor][KernelRun]tag[%s], all_gather failed, return[%d]",
        ...
        if (level1RankSize > 1) {
            ...
            CHK_RET(ExecutorBase::ExecEmptyTask(execMem.inputMem, execMem.outputMem, const_cast<Stream&>(param.stream),
                dispatcher_));
            HCCL_INFO("[CollAllGatherDoubleRingAsymExecutor] level1 run success");
        }
